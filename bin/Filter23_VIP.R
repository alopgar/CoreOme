...
OOB_table$RLoss <- 100-(OOB_table$N_reads/OOB_table[1,"N_reads"])*100
...
set.seed(12)
trf_bad <- tuneRF(x = rfmdat_bad[, !names(rfmdat_bad) %in% RanForFactor], y = rfmdat_bad[,RanForFactor], ntree = 10000, 
...
set.seed(14)
rfa_bad <- randomForest(rfformula, data = rfmdat_bad, mtry = tune_mtry, ntree = rf_ntree, nodesize = 3, importance = T)
...
ra_bad <- ra_all[rownames(ra_all) %in% rownames(vImp_bad),]
ra_1perc <- ra_all[rownames(ra_all) %in% names(vImp_bad_top),]

tbl.report(ra_bad[1:8,1:6])
write.report(paste("Low prevalence features:", dim(ra_bad)[1], "features within", dim(ra_bad)[2]-4, "samples"))
...
# Create non-core table using table with GLM+mean_RA+prev+single/doubletons, non-core counts and taxonomy/functions tables:
...
if(isTRUE(glmfilter)){
	lpstats <- c(dim(ra_all)[1], dim(ra_bad)[1], dim(glm_ra_pv)[1], dim(sgtp_bad_prev)[1], dim(sgtp_bad_good)[1])
	names(lpstats) <- c("Total", paste0("< ", OOB_min$Threshold, "% Prevalence"), "Importance > 1%", paste("Prevalence >", prevfilter), "LM p-value < 0.05")
} else if(isFALSE(glmfilter)){
	lpstats <- c(dim(ra_all)[1], dim(ra_bad)[1], dim(glm_ra_pv)[1], dim(sgtp_bad_good)[1])
	names(lpstats) <- c("Total", paste0("< ", OOB_min$Threshold, "% Prevalence"), "Importance > 1%", paste("Prevalence >", prevfilter))
}
...
if (datatype == "taxonomy"){
	...
	write.table(final_good %>% rownames_to_column("Feat"), paste0(WD, "Filtered/", "SQMr_filtered_kegg.csv"), sep='\t', quote = F, row.names = F)
	write.table(sgtp_imp, paste0(WD, "Filtered/", "SQMr_important_Lprev_kegg.csv"), sep='\t', quote = F, row.names = F)
}
...
# Just in case: save RF objects into R file...
#save(rfa_bad, file = paste0(WD, "Filtered/", "RandomForestItem.rda"))
