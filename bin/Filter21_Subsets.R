...
if (datatype == "taxonomy"){
	...
	sgt_mdata_pre <- read.delim(paste0(WD, "Filtered/", "F1.mreads.tsv"), stringsAsFactors = F)
	# Remove combined keggs:
	write.report(c("\n", "--- ADDITIONAL KEGG FILTER: Removing multi-named keggs", ""))
	bad_keggs <- sgt_mdata_pre[str_detect(sgt_mdata_pre$KEGG, "_"),]
	sgt_mdata <- sgt_mdata_pre[-which(sgt_mdata_pre$KEGG %in% bad_keggs$KEGG),]
	write.report(c("", "- Number of keggs removed:", nrow(bad_keggs)))
}
